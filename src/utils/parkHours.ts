...
// Summer closes at midnight (0:00), handled via the "before OPEN_HOUR" check.
 ...
 *
 * Schedule:
 *  - April–September: 6:00 – 0:00 (midnight)
 *  - October–March:   6:00 – 22:00
 */
export function resolveParkHours(now?: Date): ParkHoursInfo {
  ...
  const closeTime = summer ? "0:00" : "22:00";

  // Before opening → closed for night
  if (hour < OPEN_HOUR) {
    return { state: "closed_for_night", openTime: "6:00", closeTime };
  }

  if (summer) {
    // Summer: closes at midnight (0:00).
    // 23:00–23:59 → closing soon
    if (hour === 23) {
      return { state: "closing_soon", openTime: "6:00", closeTime };
    }
    // 6:00–22:59 → open
